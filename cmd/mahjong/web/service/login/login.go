...
func MakeHandler() http.Handler {
	...
	router.Handle("/v1/user/logout", nex.Handler(userLogoutHandler)).Methods("GET")         //用户下线
	return router
}
...
func thirdUserLoginHandler(r *http.Request, data *protocol.ThirdUserLoginRequest) (*protocol.LoginResponse, error) {
	...
	if err != nil && err != errutil.YXErrThirdAccountNotFound {
		logger.Error(err)
		return nil, err
	}
	//用户存在
...
func userLogoutHandler(query *nex.Form) (*protocol.StringMessage, error) {
	...
	u, err := db.QueryUser(um.Uid)
	if err != nil {
		return nil, err
	}
	u.IsOnline = db.UserOffline
	db.UpdateUser(u)
